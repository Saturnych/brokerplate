...
const brokerConfig: BrokerOptions = {
	...
	nodeID: !!process.env.NODEID ? process.env.NODEID : null, // `${process.env.NODEID}.${v4()}`
	// Custom metadata store. Store here what you want. Accessing: `this.broker.metadata`
	metadata: {
		region: 'eu-west1',
	},
	// Custom UID generator function for Context ID.
	uidGenerator: v4,
	...
	logLevel: process.env.LOGLEVEL
		? (process.env.LOGLEVEL as LogLevels)
		: 'info',
	...
	serializer: process.env.SERIALIZER ? process.env.SERIALIZER : 'JSON',
	...
	retryPolicy: {
		// Enable feature
		enabled: String(process.env.FAULTRETRY) === 'true',
		// Count of retries
		retries: 5,
	...
	tracking: {
		// Enable feature
		enabled: String(process.env.CONTEXTTRACKING) === 'true',
	...
	registry: {
		...
		strategy: process.env.STRATEGY ? process.env.STRATEGY : 'RoundRobin',
	...
	circuitBreaker: {
		// Enable feature
		enabled: String(process.env.CIRCUITBREAKER) === 'true',
	...
	bulkhead: {
		// Enable feature.
		enabled: String(process.env.FAULTBULKHEAD) === 'true',
	...
	metrics: {
		enabled: !!process.env.METRICS,
		// Available built-in reporters: 'Console', 'CSV', 'Event', 'Prometheus', 'Datadog', 'StatsD', 'Laboratory'
		reporter: process.env.METRICS,
	},
	...
	created: async (broker: ServiceBroker): Promise<void> => {
		if (DEBUG) broker.logger.info('broker created');
	},
	// Called after broker started.
	started: async (broker: ServiceBroker): Promise<void> => {
		if (DEBUG) broker.logger.info('broker started');
	},
	// Called before broker stopped.
	stopped: async (broker: ServiceBroker): Promise<void> => {
		if (DEBUG) broker.logger.info('broker stopped');
	},
};
