		...
		const schema: ServiceSchema = {
						...
						'/': {
							...
							packetMiddlewares: [
								...
								(packet, next) => {
									...
									this.logger.info(
										'io namespace packet middleware'
									); //point to service instance.
									next();
								},
							],
							events: {
								call: {
									...
									onBeforeCall: async (
										...
										ctx.service.logger.info(
											'io before hook:',
											{ action, params, callOptions }
										);
									},
									onAfterCall: async (ctx, socket, res) => {
										//after hook
										ctx.service.logger.info(
											'io after hook:',
											res
			...
			methods: {
				async socketAuthorize(socket, eventHandler): Promise<any> {
					...
					if (token) {
						return this.broker.call(`${VERSION}.auth.access`, {
							token,
						});
					} else {
						// anonymous user
