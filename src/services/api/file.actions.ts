...
export const GetFile = async ({ res, filename, originalname = '', contentType = 'image/jpeg', download = false }) => new Promise((resolve, reject) => {
    const ctx = res.$ctx;
    if (!!!filename || !!!contentType) return reject(new Error('Wrong filename or contentType!'));
		const filePath = join(
			__dirname,
			'../../../public',
			filename
    ...
    readStream.on('error', (err) => {
      if (ctx.service.debug()) ctx.service.logger.error('createReadStream() error:', err.message);
			reject(err.message);
    });
    readStream.on('end', () => {
      if (ctx.service.debug()) ctx.service.logger.info('createReadStream() end!');
      resolve(readStream);
    });
    readStream.pipe(res);
});
	...
	get: {
		...
		handler: async (
			ctx: Context<any, { filename: string }>
				...
				const params = Object.assign({ filename, originalname, mime, download }, ctx.meta['$params'], ctx.meta);
