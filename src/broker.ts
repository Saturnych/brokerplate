...
const serviceDir = (!!SERVICEDIR && existsSync(SERVICEDIR))?SERVICEDIR:'./services';

let services: Record<string, any> = {};
if (serviceList.length>0) {
  serviceList.forEach(serv => {
    services[serv.split('.')[0] as string] = require(`${serviceDir}/${serv}`).default;
  });
} else {
...
for (const [name, service] of Object.entries(services)) broker.createService(service);

broker.start().then(() => {
  broker.logger.warn(`++++ ${moleculerConfig.nodeID} services broker started ++++`);
  // Switch to REPL mode at development
  if (DEV_MODE) broker.repl();
}).catch(err => {
  if (broker) broker.logger.error(`---- ${moleculerConfig.nodeID} services broker error: ${err} ----`);
});
